...
const IMAGES_DIR = './public/boticario';
const PROGRESS_FILE = './progress.json';
const TOTAL_IMAGES = 199;
const DELAY_MS = 1000;
...
async function insertPage(pagina, arquivo, texto, embedding) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    const { error } = await supabase
      .from('catalogo_boticario')
      .insert({ pagina, marca: 'O Boticário', arquivo, texto, embedding });
...
async function clearTable() {
  console.log('Limpando tabela catalogo_boticario...');
  const { error } = await supabase
    .from('catalogo_boticario')
    .delete()
    .gte('pagina', 0);
...
async function main() {
  ...
  console.log(`Iniciando ingestão de ${TOTAL_IMAGES} imagens...\n`);

  for (let i = 0; i < TOTAL_IMAGES; i++) {
    const arquivo = `catalogo_boticario_abril${i}.jpg`;
    const pagina = i + 1;
    ...
    if (!fs.existsSync(imagePath)) {
      console.warn(`[${pagina}/${TOTAL_IMAGES}] Arquivo não encontrado — pulando.`);
      continue;
    }
